varying vec3 normal, eyeVec, halfVector, lightDir, halfVectorPoint, lightDirPoint, lightDirD;
varying float att;
...
void main (void)
{
	...
	float cutoff = cos(gl_LightSource[1].spotCutoff);
	...
	if( cos_cur_angle > cos_outer_cone_angle)
	{
		...
		if(lambertTerm > 0.0)
		{
			...
			final_color += gl_LightSource[1].specular *	gl_FrontMaterial.specular *specular *att;	
		}
	}
